...
export default function ExportImport() {
  ...
  const exportData = async (exportFormat: 'csv' | 'pdf') => {
    ...
    try {
      ...
      showToast(t('errorExporting'), 'error')
    } finally {
  ...
  const parseCSV = useCallback((text: string) => {
    ...
    if (headers.length < 2 || headers[0] !== expectedHeaders[0] || headers[1] !== expectedHeaders[1]) {
      setParseErrors([t('csvInvalidHeader', { headers: expectedHeaders.join(', ') })])
      setParsedRows([])
      return
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (!date.match(/^\d{4}-\d{2}-\d{2}$/)) {
        errors.push(t('csvInvalidDate', { row: i, date }))
        continue
      }
      if (!['office', 'home', 'off', 'holiday', 'sick'].includes(type)) {
        errors.push(t('csvInvalidType', { row: i, type }))
        continue
      }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast(t('importedToast', { imported: data.imported, updated: data.updated }), 'success')
      } else {
        const data = await response.json()
        showToast(data.error || t('importFailed'), 'error')
      }
    } catch {
      showToast(t('errorImporting'), 'error')
    } finally {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-4 py-6 pb-[calc(1.5rem+var(--sai-bottom))] sm:py-12 sm:px-6 lg:px-8">
        ...
        <div className="space-y-6 card p-4 sm:p-8">
          <h3 className="text-lg font-semibold text-text-primary">{t('exportReport')}</h3>
          <div>
          ...
          <div className="mt-6 rounded-lg border border-border bg-surface-secondary p-4">
            <h3 className="mb-2 text-sm font-semibold text-text-primary">{t('exportInfo')}</h3>
            <ul className="space-y-1 text-xs text-text-secondary">
              <li>{'• '}{t('csvFormat')}</li>
              <li>{'• '}{t('pdfFormat')}</li>
              <li>{'• '}{t('exportUseCase')}</li>
            </ul>
          </div>
        </div>
        ...
        <div className="mt-8 space-y-6 card p-4 sm:p-8">
          <h3 className="text-lg font-semibold text-text-primary">{t('importCSV')}</h3>
          ...
          {parseErrors.length > 0 && (
            <div className="rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-900 dark:bg-red-900/20">
              <h4 className="mb-2 text-sm font-semibold text-red-800 dark:text-red-200">
                {t('parsingErrors', { count: parseErrors.length })}
              </h4>
              ...
              <div className="max-h-64 overflow-auto rounded-lg border border-border">
                <table className="w-full text-left text-xs">
                  <thead className="sticky top-0 bg-surface-secondary">
                    <tr>
                      <th className="px-3 py-2 font-medium text-text-secondary">{t('date')}</th>
                      <th className="px-3 py-2 font-medium text-text-secondary">{t('type')}</th>
                      <th className="px-3 py-2 font-medium text-text-secondary">{t('location')}</th>
                      <th className="px-3 py-2 font-medium text-text-secondary">{t('transport')}</th>
                      <th className="px-3 py-2 font-medium text-text-secondary">{t('notes')}</th>
                    </tr>
                  </thead>
          ...
          {importResult && (
            <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-900 dark:bg-emerald-900/20">
              <p className="text-sm font-medium text-emerald-800 dark:text-emerald-200">
                {t('importComplete', { imported: importResult.imported, updated: importResult.updated })}
              </p>
            </div>
          )}

          <div className="rounded-lg border border-border bg-surface-secondary p-4">
            <h4 className="mb-2 text-sm font-semibold text-text-primary">{t('importInfo')}</h4>
            <ul className="space-y-1 text-xs text-text-secondary">
              <li>{'• '}{t('csvHeaders')}</li>
              <li>{'• '}{t('csvOverwrite')}</li>
              <li>{'• '}{t('csvMatch')}</li>
              <li>{'• '}{t('csvTemplate')}</li>
            </ul>
          </div>
